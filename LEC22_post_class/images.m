...
% Top border
im1(1:10, :, 1) = uint8(255); % red channel
im1(1:10, :, 2:3) = uint8(0); % green and blue channels

% Bottom border
im1(rows-10:rows, :, 2) = uint8(255); % green channel
im1(rows-10:rows, :, [1,3]) = uint8(0); % red and blue channels

% Right border
im1(:, cols-10:cols, 3) = uint8(255); % blue channel
im1(:, cols-10:cols, 1:2) = uint8(0); % red and green channels

% Left border
im1(:, 1:10, 1) = uint8(255); % red channel
im1(:, 1:10, 2) = uint8(255); % green channel
im1(:, 1:10, 3) = uint8(0); % blue channel
